...
public class FrameHeader
{
    ...
    public FrameHeader(byte[] frameHeaderBytes)
    {
        ...
        Flags = (
            preservation: (frameHeaderBytes[4] & 0b1000_000) != 0,
    ...
    private static bool ValidateFrameHeader(byte[] frameHeaderBytes) =>
        frameHeaderBytes.Length == 10 &&
        frameHeaderBytes.Take(4).All(b => (b >= 48 && b <= 57) || (b >= 65 && b <= 90));

    private static int ParseLength(IEnumerable<byte> headerLengthBytes)
    {
    ...
    public override string ToString() => $"{FrameID}, Length: {Length}";
}
